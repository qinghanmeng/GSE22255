...
POINT **allocate_point_array(POINT **array_name, int size, int prev_size)
{
  ...
  if (prev_size == 0)
      ...
      for (i=1;i<=size;i++)
	{
      ...
      for (i=1;i<=size;i++)
	array_name[i]->dimension = allocate_vector(1,no_of_dimensions);
    }
  else
    {
      array_name += 1;
...
/* Returns : Nothing.                                                   */
/* Calls modules : allocate_vector (util.c)                             */
/*                 free_vector (util.c)                                 */
/*                 average (util.c)                                     */
...
/*                            Defined in oc1.h.                         */
/************************************************************************/
fill_missing_values(struct point **points,int no_of_points)

{
  int i,j,count;
  float avg,average(),*temp;
  
  temp = allocate_vector(1,no_of_points);
